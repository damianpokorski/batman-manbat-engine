...
namespace Manbat{
	...
	cSound::cSound(char* filename){
		...
		HRESULT hr = m_pWaveSoundRead->Open(filename);
		if(FAILED(hr)){
			Debug << "Couldn't open the sound file " << filename << "\n";
		}
		Init();
		Fill();
	}
	...
	void cSound::Restore(){
		...
		if(FAILED(hr= m_pBuffer->GetStatus(&dwStatus))){
			Debug << "[cSound] SoundBuffer query to 8 failed! \n";
		}
	...
	void cSound::Init(){
		...
		if(FAILED(hr)){
			Debug << "[cSound] CreateSoundBuffer failed \n";
		}
		
		pTempBuffer->QueryInterface(IID_IDirectSoundBuffer8, (void**)&m_pBuffer);
		if(FAILED(hr)){
			Debug << "[cSound] SoundBuffeer Query failed \n";
		}
	...
	void cSound::Fill(){
		...
		if(NULL==pbWavData){
			delete [] pbWavData;
			Debug << "[cSound] Out of memory!\n";
		}
		hr = m_pWaveSoundRead->Read(nWaveFileSize, pbWavData, &cbWavSize);
		if(FAILED(hr)){
			delete [] pbWavData;
			Debug << "[cSound] m_pWaveSoundRead->read failed \n";
		}
		...
		if(FAILED(hr)){
			delete [] pbWavData;
			Debug << "[cSound] m_pBuffer->lock failed";
		}
	...
	bool cSound::IsPlaying(){
		DWORD dsStatus = 0;
		m_pBuffer->GetStatus(&dsStatus);
		// could just return (dsStatus &DSBSTATUS_PLAYING) but w/e
		if(dsStatus &DSBSTATUS_PLAYING)
			return true;
		else 
			return false;
	}
